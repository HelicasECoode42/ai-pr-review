...
- 控制模型上下文预算。
- 使用 cl100k_base 计算 token；编码不可用时使用保守 UTF-8 byte 上界降级。
...
默认 budget：patch 上下文 6,000 tokens + Context Pack 1,000 tokens。

- **预算可解释**：英文与中文不再共用含义不一致的字符阈值，报告记录实际 context token 数与截断状态。
- **成本可控**：prompt 上界可直接映射到不同 provider 的实时计价，不在代码中固化易过期的价格假设。
- **响应速度**：优先保留风险文件并限制 prompt，减少无关上下文带来的模型推理开销。
